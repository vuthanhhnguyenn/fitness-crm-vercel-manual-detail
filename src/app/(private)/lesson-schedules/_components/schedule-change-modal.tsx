...
import {
  NO_CHANGE_INSTRUCTOR,
  type ScheduleChangeFormSubmitValues,
  type ScheduleChangeFormValues,
  scheduleChangeFormSchema,
} from '../_schemas/schedule-change-form.schema';
import {
  formatTimeRange,
  getLessonTypeLabel,
  toTimeSlot,
...
export function ScheduleChangeModal({
  ...
  schedule,
  onOpenChange,
}: Readonly<ScheduleChangeModalProps>) {
  return (
...
function ScheduleChangeModalContent({
  schedule,
  onOpenChange,
}: Readonly<ScheduleChangeModalContentProps>) {
  ...
  const form = useForm<ScheduleChangeFormValues, unknown, ScheduleChangeFormSubmitValues>({
    resolver: zodResolver(scheduleChangeFormSchema) as never,
    mode: 'onChange',
    defaultValues: {
      ...
      endTime: originalEnd,
      instructorId: NO_CHANGE_INSTRUCTOR,
  ...
  const instructorSelectItems = useMemo(
    () => [
      { value: NO_CHANGE_INSTRUCTOR, label: '変更なし' },
      ...instructorOptions.map((i) => ({ value: i.id, label: i.name })),
    ],
    [instructorOptions],
  );
  ...
  const studioSelectItems = useMemo(
    () =>
      schedule.studio_name ? [{ value: schedule.studio_name, label: schedule.studio_name }] : [],
    [schedule.studio_name],
  );

  const [reasonValue, startTimeValue, endTimeValue, changeScope] = useWatch({
    control: form.control,
    name: ['reason', 'startTime', 'endTime', 'changeScope'],
  });
  const isTimeRangeValid = startTimeValue < endTimeValue;
  const canSubmit = !!reasonValue?.trim() && isTimeRangeValid;
  ...
  const changeMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({
        queryKey: getCrmLessonSchedulesStoresSummaryQueryKey(),
      });
      onOpenChange(false);
    },
    onSettled: () => {
      ...
      (values) => {
        changeMutation.mutate({
          ...
          body: {
            ...
            new_start_time: values.startTime !== originalStart ? values.startTime : undefined,
            new_end_time: values.endTime !== originalEnd ? values.endTime : undefined,
            new_instructor_id:
              values.instructorId !== NO_CHANGE_INSTRUCTOR ? values.instructorId : undefined,
          },
        });
      },
      () => {
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <FormField
                    control={form.control}
                    name="startTime"
                    render={({ field }) => <Input type="time" className="h-9 text-sm" {...field} />}
                  />
                </div>
                <div>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    ...
                    >
                      ...
                      <SelectContent>
                        <SelectItem value={NO_CHANGE_INSTRUCTOR}>変更なし</SelectItem>
          ...
          <div className="flex w-full items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <Button
                ...
                disabled={isPending || !canSubmit || form.formState.isSubmitting}
              >
                {isPending ? '変更中...' : '変更を確定'}
              </Button>
            </div>
