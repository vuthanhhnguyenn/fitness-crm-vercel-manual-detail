...
import { CRM_MAINTENANCE_STATUS_LABELS } from '@/app/(private)/crm-maintenance/_constants/crm-maintenance.constants';
import type { useCrmMaintenanceFilters } from '@/app/(private)/crm-maintenance/_hooks/use-crm-maintenance-filters.hook';
...
interface CrmMaintenanceFiltersProps {
  isFilterOpen: boolean;
  onFilterOpenChange: (open: boolean) => void;
  filtersHook: ReturnType<typeof useCrmMaintenanceFilters>;
}

export function CrmMaintenanceFilters({
  isFilterOpen,
  onFilterOpenChange,
  filtersHook,
}: CrmMaintenanceFiltersProps) {
  const { filters, setFilters, searchInput, setSearchInput, activeFilterCount } = filtersHook;

  const STATUS_FILTER_ITEMS = { all: '全ステータス', ...CRM_MAINTENANCE_STATUS_LABELS };
      ...
      {isFilterOpen && (
        <div className="flex flex-wrap items-center gap-2">
          <Select
            ...
            onValueChange={(value) =>
              setFilters({
                status: value === 'all' ? null : (value as CrmMaintenanceStatus),
                page: 1,
              })
            }
          >
