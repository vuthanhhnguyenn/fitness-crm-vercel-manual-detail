...
import {
  deleteCrmOptionDiscountsByIdMutation,
  getCrmOptionDiscountsQueryKey,
} from '@/lib/api/@tanstack/react-query.gen';
import { navigate } from '@/lib/routes/routes.util';

const DeleteOptionDiscountSchema = z.object({
  reason: z.string().min(1, '削除理由は必須です'),
});

type DeleteOptionDiscountValues = z.infer<typeof DeleteOptionDiscountSchema>;

interface OptionDiscountDeleteDialogProps {
...
export function OptionDiscountDeleteDialog({
  ...
  const form = useForm<DeleteOptionDiscountValues>({
    resolver: zodResolver(DeleteOptionDiscountSchema),
    mode: 'onChange',
  ...
  const onSubmit = (values: DeleteOptionDiscountValues) => {
    deleteMutation.mutate({
      path: { id: optionDiscountId },
  ...
  return (
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>セット割を削除しますか？</AlertDialogTitle>
          <AlertDialogDescription>
            <span className="text-foreground font-medium">{optionDiscountName}</span>{' '}
            を削除します。この操作は取り消せません。
          </AlertDialogDescription>
        </AlertDialogHeader>
