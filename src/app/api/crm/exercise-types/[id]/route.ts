...
const path = '/crm/exercise-types/{id}';
const idParam = {
  name: 'id',
  in: 'path' as const,
  required: true,
...
registerRoute({
  method: 'get',
  path,
  summary: 'Get exercise type detail',
  description: 'Get a specific exercise type record',
  tags: ['Exercises'],
  parameters: [idParam],
  responses: [
...
registerRoute({
  ...
  description: 'Update a specific exercise type record',
  tags: ['Exercises'],
  parameters: [idParam],
  requestBody: {
...
registerRoute({
  ...
  description: 'Logical delete for exercise type records',
  tags: ['Exercises'],
  parameters: [idParam],
  responses: [
...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const item = db.exerciseMasters.getById('exercise_type', id);
    if (!item) {
    ...
    console.error('GET exercise type exercise master detail error:', error);
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const item = db.exerciseMasters.update('exercise_type', id, validationResult.data);
    if (!item) {
      return NextResponse.json({ error: '参照マスタが見つかりません' }, { status: 404 });
    }
    ...
    console.error('PATCH exercise type exercise master error:', error);
...
export async function DELETE(
  ...
  try {
    const { id } = await params;
    const result = db.exerciseMasters.delete('exercise_type', id);
    if (!result) {
      return NextResponse.json({ error: '参照マスタが見つかりません' }, { status: 404 });
    }
    ...
    console.error('DELETE exercise type exercise master error:', error);
