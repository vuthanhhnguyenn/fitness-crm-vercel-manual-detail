...
const path = '/crm/exercise-muscles/{id}';
const idParam = {
  name: 'id',
  in: 'path' as const,
  required: true,
...
registerRoute({
  method: 'get',
  path,
  summary: 'Get muscle detail',
  description: 'Get a specific muscle record',
  tags: ['Exercises'],
  parameters: [idParam],
  responses: [
...
registerRoute({
  ...
  description: 'Update a specific muscle record',
  tags: ['Exercises'],
  parameters: [idParam],
  requestBody: {
...
registerRoute({
  ...
  description: 'Logical delete for muscle records',
  tags: ['Exercises'],
  parameters: [idParam],
  responses: [
...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const item = db.exerciseMasters.getById('muscle', id);
    if (!item) {
    ...
    console.error('GET muscle exercise master detail error:', error);
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const item = db.exerciseMasters.update('muscle', id, validationResult.data);
    if (!item) {
      return NextResponse.json({ error: '参照マスタが見つかりません' }, { status: 404 });
    }
    ...
    console.error('PATCH muscle exercise master error:', error);
...
export async function DELETE(
  ...
  try {
    const { id } = await params;
    const result = db.exerciseMasters.delete('muscle', id);
    if (!result) {
      return NextResponse.json({ error: '参照マスタが見つかりません' }, { status: 404 });
    }
    ...
    console.error('DELETE muscle exercise master error:', error);
