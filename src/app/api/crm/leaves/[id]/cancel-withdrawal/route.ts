...
registerRoute({
  ...
  description: 'Cancel a withdrawal_scheduled request and revert the member status back to active',
  tags: ['Leaves'],
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const leave = db.memberLeaves.cancelWithdrawal(id, parsed.data.comment);
  if (!leave) {
    return NextResponse.json(
      { error: `Cannot cancel withdrawal with status: ${existing.status}` },
      { status: 400 },
    );
  }
